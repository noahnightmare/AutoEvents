...
namespace AutoEvents.Events.HideAndSeekEntrance
{
    ...
    public class HideAndSeekEntrance : Event
    {
        // Set the info for the event.
        public override string Name { get; set; } = "HideAndSeekEntrance";
        public override EventType eventType { get; set; } = EventType.Event;
        public override string CommandName { get; set; } = "hase";
        ...
        protected override void OnStart()
        {
            ...
            Map.Broadcast(200, "<b>Small Hide And Seek [ENTRANCE]\n<color=orange>Be the last Class D remaining!</color>");

            foreach (Door door in Door.List.Where(d => d.IsCheckpoint || d.IsPartOfCheckpoint || d.Type == DoorType.GateA || d.Type == DoorType.GateB))
            {
                door.IsOpen = false;

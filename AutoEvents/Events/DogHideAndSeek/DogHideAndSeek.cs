...
namespace AutoEvents.Events.DogHideAndSeek
{
    ...
    public class DogHideAndSeek : Event
    {
        // Set the info for the event.
        public override string Name { get; set; } = "DogHideAndSeek";
        public override EventType eventType { get; set; } = EventType.Event;
        public override string CommandName { get; set; } = "dhas";
        ...
        protected override void OnStart()
        {
            ...
            randomPlayer.Position = Room.Get(_config.Room).WorldPosition(_config.RelativePosition);
            ...
            Timing.CallDelayed(_config.TimeToLetHidersHide, () =>
            {
                foreach(Player player in Player.List.Where(x => x.Role == _config.SeekerRole))
                {
                    ...
                    Cassie.MessageTranslated("jam_010_2 SCP 9 3 9 pitch_0.9 has breached containment . pitch_0.9 All ClassD Personnel must jam_020_2 pitch_0.7 run pitch_0.8 immediately . ",
                        "<color=red>SCP-939 has breached containment.</color> All ClassD Personnel must run immediately.");
                }
            });

            randomPlayer.ClearBroadcasts();
            randomPlayer.Broadcast((ushort)_config.TimeToLetHidersHide, $"<b>You have been chosen as the starting 939!\n<color=red>Kill everyone.\nYou are frozen for {_config.TimeToLetHidersHide} seconds to let people hide.</color></b>");

            Map.Broadcast(200, "<b>Dog Hide And Seek\n<color=orange>Be the last Class D remaining!</color></b>");
        ...
        protected override void ProcessEventLogic()
        {
            ...
            {
                player.Role.Set(_config.SeekerRole);
